 ...
 * Fetches all the packages and puts them into `Package` objects
 */
Future<List<String>> fetchAllPackageNames() {
  return fetchAllPackagePages().then((List<PackagePage> pubPackages) {
   List<String> packageNames = <String> [];
   pubPackages.forEach((PackagePage packagePage) {
     var packagePageNames = packagePage.packages.map((uri) => new RegExp(r'([^/]*).json$').firstMatch(uri).group(1));
     packageNames.addAll(packagePageNames);
   });
